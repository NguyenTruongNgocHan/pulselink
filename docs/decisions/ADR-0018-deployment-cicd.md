# ADR-0018: Render API + Render Key Value + Vercel web + Supabase, GitHub Actions CI
...
- **Database + private attachment storage** → Supabase (already decided in
  ADR-0002/0003) — no change here, just confirming it's part of the same
  deployment picture.
- **Redis-compatible demo store** → **Render Key Value** in the same Render
  region as the API. New instances use Valkey, which is Redis-protocol
  compatible; Spring connects through `REDIS_URL`. Local development continues
  to use the official Redis container. This store contains only reconstructable
  presence and rate-limit state, so free-demo persistence limitations do not
  threaten message durability.
- **CI**: a GitHub Actions workflow on every push/PR to `main`:
  ...
  2. Frontend job: `npm ci && npm run lint && npm run test -- --run && npm run build`.
...
- Secrets (JWT signing key, Supabase credentials, `REDIS_URL`, Render deploy hook URL,
...
- **Upstash Redis, Railway, or Fly.io instead of Render Key Value/API hosting** — comparable Docker-based hosts;
...
- CI runs backend tests plus frontend lint/component tests/build — no automated
