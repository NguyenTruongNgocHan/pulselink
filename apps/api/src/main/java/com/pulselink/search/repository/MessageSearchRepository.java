    ...
    public List<MessageSearchResponse> search(
        ...
        return jdbc.query("""
                select m.id,m.content,m.created_at,m.edited_at,
                       u.display_name sender_name,u.username sender_username,
                       c.id conversation_id,c.type,
                       case when c.type='GROUP' then c.name else
                         (select other.display_name from conversation_participants other_p
                          join users other on other.id=other_p.user_id
                          where other_p.conversation_id=c.id and other_p.user_id<>?
                            and other_p.left_at is null limit 1)
                ...
                where cp.user_id=? and cp.left_at is null
                  and m.deleted_at is null and m.moderated_at is null
                ...
                rs.getString("type")
        ), userId, userId, query, conversationId, conversationId, query);
    }
