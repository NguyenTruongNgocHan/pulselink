...
function mapConversation(dto: UnknownRecord): ConversationSummary {
  const participants = Array.isArray(dto.participants)
    ? dto.participants.map((item) => mapParticipant(item as UnknownRecord))
    : []

  return {
    id: text(dto.id),
    type: text(dto.type) === 'GROUP' ? 'GROUP' : 'DIRECT',
    name: text(dto.name ?? dto.displayName ?? dto.display_name, 'Conversation'),
    avatarUrl: (dto.avatarUrl ?? dto.avatar_url ?? dto.avatar_object_key ?? null) as string | null,
    status: text(dto.status) === 'CLOSED' ? 'CLOSED' : 'ACTIVE',
    preview: (dto.preview ?? dto.latestMessage ?? dto.latest_message ?? null) as string | null,
    latestMessageAt: (dto.latestMessageAt ?? dto.latest_message_at ?? dto.updated_at ?? null) as
      | string
      | null,
    unreadCount: numberValue(dto.unreadCount ?? dto.unread_count),
    memberCount: numberValue(dto.memberCount ?? dto.member_count, participants.length),
    participants,
  }
}
...
function mapAttachment(dto: UnknownRecord): MessageAttachment {
  return {
    ...
    mimeType: text(dto.mimeType ?? dto.mime_type, 'application/octet-stream'),
    sizeBytes: numberValue(dto.sizeBytes ?? dto.size_bytes),
    downloadUrl:
      resolveApiUrl(
        text(dto.downloadUrl ?? dto.download_url ?? dto.url, `/api/v1/files/${text(dto.id)}`),
      ) ?? '',
  }
}
...
function mapReaction(dto: UnknownRecord): MessageReaction {
  return {
    ...
    reactedByMe: booleanValue(dto.reactedByMe ?? dto.reacted_by_me),
  }
}
...
function mapReceipt(dto: UnknownRecord): MessageReceipt {
  return {
    ...
    displayName: text(dto.displayName ?? dto.display_name, 'Member'),
...
export function mapMessage(dto: UnknownRecord): Message {
  return {
    id: text(dto.id),
    conversationId: text(dto.conversationId ?? dto.conversation_id),
    senderId: text(dto.senderId ?? dto.sender_id),
    senderName: text(dto.senderName ?? dto.sender_name, text(dto.username, 'Member')),
    senderUsername: text(dto.senderUsername ?? dto.sender_username ?? dto.username),
    senderAvatarUrl: (dto.senderAvatarUrl ?? dto.sender_avatar_url ?? null) as string | null,
    ...
    deletedAt: (dto.deletedAt ?? dto.deleted_at ?? null) as string | null,
    moderatedAt: (dto.moderatedAt ?? dto.moderated_at ?? null) as string | null,
    clientMessageId: (dto.clientMessageId ?? dto.client_message_id ?? null) as string | null,
    attachments: Array.isArray(dto.attachments)
      ? dto.attachments.map((item) => mapAttachment(item as UnknownRecord))
      : [],
    reactions: Array.isArray(dto.reactions)
      ? dto.reactions.map((item) => mapReaction(item as UnknownRecord))
      : [],
    receipts: Array.isArray(dto.receipts)
      ? dto.receipts.map((item) => mapReceipt(item as UnknownRecord))
      : [],
    savedByMe: booleanValue(dto.savedByMe ?? dto.saved_by_me),
  }
}

export async function getConversations(): Promise<ConversationSummary[]> {
  const { data } = await apiClient.get<UnknownRecord[]>('/api/v1/conversations')
  return data.map(mapConversation)
}

export async function getConversation(conversationId: string): Promise<ConversationSummary> {
  const { data } = await apiClient.get<UnknownRecord>(`/api/v1/conversations/${conversationId}`)
  return mapConversation(data)
}

export async function createDirectConversation(personId: string): Promise<ConversationSummary> {
...
export async function editMessage(messageId: string, content: string): Promise<Message> {
  const { data } = await apiClient.patch<UnknownRecord>(`/api/v1/messages/${messageId}`, {
    content,
  })
  return mapMessage(data)
}
...
export async function reactToMessage(messageId: string, emoji: string): Promise<void> {
  await apiClient.put(`/api/v1/messages/${messageId}/reaction`, { emoji })
}

export async function removeReaction(messageId: string): Promise<void> {
  await apiClient.delete(`/api/v1/messages/${messageId}/reaction`)
}
...
export async function uploadAttachment(
  ...
  const { data } = await apiClient.post<UnknownRecord>('/api/v1/files', formData, {
    signal,
